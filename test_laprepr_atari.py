...
def main(hyperparams):
    ...
    count_rep = jnp.where(
        jnp.all(obs == agent_color, axis=-1, keepdims=True), ones, zeros
    ).sum(0)
    agent_loc = jnp.tile(
        jnp.all(obs == agent_color, axis=-1, keepdims=True), (1, 1, 1, 3)
    )
    ...
    for i in tqdm(range(0, d)):
        ...
        mean_rep = (
            (sum_rep / (count_rep + 1e-6)).astype(np.uint8).clip(0, 255)
        )  # agent_loc_fn()
