...
class GeneralizedAugmentedLagrangianTrainer(LaplacianEncoderTrainer, ABC):
    ...
    def compute_graph_drawing_loss(self, start_representation, end_representation):
        ...
        graph_induced_norm_dict = {
            f"graph_norm({i})": graph_induced_norms[i] for i in range(self.d)
        }
    ...
    def compute_orthogonality_loss(self, params, error_matrix_dict):
        ...
        barrier_loss = (
            jax.lax.stop_gradient(barrier_coefficients) * quadratic_error_matrix
        ).sum()
    ...
    def loss_function(self, params, train_batch, **kwargs) -> Tuple[jnp.ndarray]:
        ...
        # Update error estimates
        error_dict, error_update = self.update_error_estimates(
            params, error_matrix_dict
        )
    ...
    def update_duals(self, params):
        ...
        updates = jnp.tril(
            error_matrix
        )  # index i only messes with 1 --> i - 1 cuz of stop grad
        ...
        updated_dual_velocities = dual_velocities + self.lr_dual_velocities * (
    ...
    @abstractmethod
    def update_barrier_coefficients(
        self, params, *args, **kwargs
    ):  # TODO: eliminate this function when the best version is found
