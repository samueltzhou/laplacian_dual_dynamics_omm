...
class NormObs(ObservationWrapper):
    def __init__(self, env, reduction_factor=1):
        ...
        if self.obs_mode in ["pixels", "both"]:
            obs_dict["pixels"] = spaces.Box(
                low=0,
                high=255,
                shape=(self.unwrapped.window_size, self.unwrapped.window_size, 3),
    ...
    def _resize_pixels(self, pixels):
        ...
        pixels = (
            jax.image.resize(pixels, shape, method=jax.image.ResizeMethod.LANCZOS3)
            .astype(jax.numpy.uint8)
            .clip(0, 255)
        )
        return pixels
