...
class GridEnv(gym.Env):
    ...
    def __init__(
        ...
        self.grid_array = 255 * self.grid[:, :, np.newaxis].repeat(
            3, axis=2
        )  # Create empty grid
        self.grid_array += 110 * (1 - self.grid[:, :, np.newaxis]).repeat(
            3, axis=2
        ...
        # print(self._dyn_mat)
        print(f"is the dynamics matrix symmetric? {np.allclose(self._dyn_mat, self._dyn_mat.T)}")
    ...
    def _get_obs(self) -> dict:
        ...
        if self.obs_mode in ["pixels", "both"]:
            ...
            obs_dict["pixels"] = (
                self._render_frame(render_mode="rgb_array", canvas=self._canvas)
                .astype(np.uint8)
                .clip(0, 255)
            )
    ...
    def reset(
        ...
        *,
        ...
        target_location_id = self.np_random.integers(
            0, self.n_states - 1, size=1, dtype=int
        )
        ...
        if self.render_mode == "human":
            self._render_frame(render_mode=self.render_mode, canvas=self._canvas)
    ...
    def render(self):
        if self.render_mode == "rgb_array":
            return self._render_frame(render_mode=self.render_mode, canvas=self._canvas)
    ...
    def _create_canvas(
        self, agent_location=None, target_location=None, use_target=None, grid_width=1
    ):
        ...
        if use_target:
            ...
            pygame.draw.rect(
                ...
                (255, 0, 0),
                pygame.Rect(
                    (pix_square_sizes * target_location.astype(float) + grid_width)[
                        ::-1
                    ],
                    pix_square_sizes[::-1],
                ),
        ...
        pygame.draw.circle(
            ...
            (0, 0, 255),
            (
                (agent_location.astype(float) + grid_width / 2) * pix_square_sizes
                + grid_width / 2
            )[::-1],
        ...
        for i, j in product(range(self.height), range(self.width)):
            if not self.grid[i, j]:
                rect_points = [
                    (
                        pix_square_sizes * np.array([i, j]).astype(float)
                        + grid_width * np.array([1, 1]) / 2
                    )[::-1],
                    (
                        pix_square_sizes * np.array([i + 1, j]).astype(float)
                        + grid_width * np.array([1, 1]) / 2
                    )[::-1],
                    (
                        pix_square_sizes * np.array([i + 1, j + 1]).astype(float)
                        + grid_width * np.array([1, 1]) / 2
                    )[::-1],
                    (
                        pix_square_sizes * np.array([i, j + 1]).astype(float)
                        + grid_width * np.array([1, 1]) / 2
                    )[::-1],
                ]
    ...
    def _render_frame(
        self, render_mode, canvas=None
    ):  # TODO: consider non-square grids
    ...
    def _maze_to_transition_tensor(self):
        ...
        )  # Transition tensor (state, action, state')
        ...
        for (row, col), state in self._state_indices.items():
            state_up, state_down, state_left, state_right = self._find_neighbors(
                row, col, state
            )
    ...
    def _compute_eigenvectors(self) -> List[np.ndarray]:
        ...
        if FLINT_INSTALLED:
            ...
            eigvals = (
                np.array(eigvals).astype(np.clongdouble).real.flatten()
            )  # real since we assume the dynamics matrix is symmetric
            eigvecs = (
                np.array(eigvecs.tolist())
                .astype(np.clongdouble)
                .real.astype(np.float32)
            )
        else:
            eigvals, eigvecs = mp.eigsy(
                mp_mat
            )  # eigsy since we assume the dynamics matrix is symmetric
